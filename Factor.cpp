...
Factor *copyFactor(Factor *fact)
{
	...
	for (long f = 0; f < newFact->tableSize; f++)
	{
...
{
	Factor *fact1 = NULL, *fact2 = NULL, *fact3 = NULL;
	...
	vec.push_back(0);
	vec.push_back(1);
	fact1->table.push_back(make_pair(vec, 6));
	vec.clear();	
	vec.push_back(1);
	vec.push_back(1);
	vec.push_back(0);
	...
	vec.push_back(1);
	vec.push_back(1);
	fact1->table.push_back(make_pair(vec, 8));
	vec.clear();	
