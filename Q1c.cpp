...
struct node
{
	...
	vector <long> psi_scope, tao_scope; 
...
void upwardPass(node *root)
{
	...
	root->tao = max_marginalize(root->psi, root->id, 10);

}

void MAP_Inference(vector <node*> &nodeSet, vector <Factor*> &initial_facts)
{
	for (auto a : nodeSet)
	{
...
void predict(node* root, vector <long> &vec_str, string finalWord,long assign)
{
	Factor *fact = root->psi;
	for (auto i : root->psi->scope)
	{
		if (vec_str[i] >= 0 && vec_str[i] <= 9)
		{
			fact = reduction(fact, i, vec_str[i], 10);
		}
	}
	double maxVal = fact->table[0].second;
	long assignval = fact->table[0].first[0];

	for (long i = 1; i < fact->tableSize; i++)
	{
		if (fact->table[i].second >= maxVal) 
		{
			maxVal = fact->table[i].second;
			assignval = fact->table[i].first[0];
		}
	}
	vec_str[root->id] = assignval; 
	for (auto r : root->children)
		predict(r, vec_str, finalWord, assignval);
}
...
string prediction(vector <node*> &nodeSet, vector <long> &vec_str, string finalWord)
{
	string str = "";
	long start_s = clock();
	double oldLL = LikeLihood;
	for (auto a : nodeSet)
		predict(a, vec_str, finalWord, -1); 
...
void findLL(node *root, string finalword)
{
	long id = char2int[finalword[root->id]];
	vector <long> vec;
	vec.push_back(root->id);

	Factor *f = sumOutVariables(root->psi, root->psi->scope, vec, 10);
	...
	for (long i = 0; i < f->tableSize; i++)
	{
		...
		if (f->table[i].first[0] == id)
			num = exp(f->table[i].second);
	}

	LikeLihood += log(num/(double)Z);
	for (auto r : root->children)
		findLL(r, finalword);
}

void find_Likelihood(vector <node*> &nodeSet, string finalword)
{
	for (auto a : nodeSet)
		findLL(a, finalword);
...
void Modelling(vector <long> image, long size1, long size2, string word1, string word2, long category)
{
	...
	MAP_Inference(nodeSet, initial_facts);

	vector <long> vec_str;
	for (long i = 0; i < image.size(); i++)
		vec_str.push_back(-1);
	string str = prediction(nodeSet, vec_str, word1 + word2);
	find_Likelihood(nodeSet, word1 + word2);
